...
# Fake tkinter like the other test to avoid opening windows
import types
...
# Prepare minimal fake instance of FullScreenMatchGUI with a real vmix URL
class SimpleGet:
...
obj.creds_path = None
# match_rows row must have at least 6 elements; position 5 holds vmix URL
row = [SimpleGet('1'), SimpleGet('A'), SimpleGet('B'), SimpleGet('10'), SimpleGet('9'), SimpleGet('http://CHUNG:8088')]
obj.match_rows = [tuple(row)]

print('Calling open_edit_popup...')
obj.open_edit_popup(0)
print('open_edit_popup finished')

# Find and invoke the captured 'Gửi' command (should be second button)
print('Captured commands count:', len(captured_commands))
if len(captured_commands) >= 2:
    print('Invoking Gửi...')
    try:
        captured_commands[1]()
    except Exception as e:
        print('Error invoking Gửi:', e)
else:
    print('Gửi command not found')
